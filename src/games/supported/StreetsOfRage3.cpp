...
//	Update terminal status
  if ((readRam(&system, 0xDFA1) == 255) && readRam(&system, 0xFB00) == 14){
      std::cout << "LOST GAME" << std::endl;
			writeRam(&system, 0xFB01, 0x0);
     // m_terminal = true;
			//writeRam(&system, 0xFB01, 0x0C);
    }
...
// Select game type
//  string game_type = system.settings()->getString("SOR3_game_type");
...
//  int level = system.settings()->getInt("SOR3_start_level");
//  if(level > 1){
...
void StreetsOfRage3Settings::startingOperations(RleSystem& system){
  ...
  if(0 == m_difficulty){
    ...
    writeRam(&system, 0xFF09, 0x0);
  }else if(1 == m_difficulty){
		writeRam(&system, 0xFF09, 0x0);
	}else if(2 == m_difficulty){
		writeRam(&system, 0xFF09, 0x2);
	}else if(3 == m_difficulty){
		writeRam(&system, 0xFF09, 0x0A);
	}else if(4 == m_difficulty){
		writeRam(&system, 0xFF09, 0x6);
	}else if(5 == m_difficulty){
		writeRam(&system, 0xFF09, 0x8);
...
//set start level	
 ...
 // }else {
	// writeRam(&system, 0xFB05, 0x01);
   //writeRam(&system, 0xFB03, 0x2);
 // }
//  if((m_start_level == 6) || (m_start_level == 7)){
